...
{  
    off64_t rest;  
    size_t c, frame_size, readPcm_size;
    ...
    while (rest > 0) {  
        ...
        readPcm_size = SNDWAV_ReadPcm(sndpcm, frame_size);
        if (readPcm_size != frame_size) {
            printf("SNDWAV_Record readPcm_size:%d; frame_size:%d.\n");
            break;
        }
